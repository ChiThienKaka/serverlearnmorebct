...
const storage = multer.diskStorage({
    destination: (req, file, cb)=>{
        ...
        if(file.fieldname === 'filevideo') {
            uploadpath = path.join(__dirname, '../../storage/lesson',lessonid,'video');
        ...
        if(file.fieldname === 'tailieu'){
            uploadpath = path.join(__dirname, '../../storage/lesson',lessonid,'tailieu');
            // tạo thư mục nếu không tồn tại 
            if(!fs.existsSync(uploadpath)){
                fs.mkdirSync(uploadpath, {recursive: true});
            }
        ...
        if(file.fieldname === 'image'){
            uploadpath = path.join(__dirname, '../../storage/lesson',lessonid,'image');
            // tạo thư mục nếu không tồn tại 
            if(!fs.existsSync(uploadpath)){
                fs.mkdirSync(uploadpath, {recursive: true});
            }
...
const uploadFileLesson = multer({storage: storage});

export default uploadFileLesson;
